...
import { ArrowRight, ArrowUpRight, Check, Mail, MapPin, MessageCircle, Phone, Plus } from "lucide-react";
...
export default function ServicePage({ pageKey, lang }: { pageKey: ServicePageKey; lang: ServiceLang }) {
  ...
  const related = servicePageKeys.filter((key) => key !== pageKey).slice(0, 4);
    ...
    <main className="service-page-shell">
      ...
      <section className="service-landing-hero">
        ...
        <div className="service-landing-content">
          <nav className="service-breadcrumb" aria-label={lang === "kz" ? "Навигация тізбегі" : "Хлебные крошки"}>
            <a href={home}>{lang === "kz" ? "Басты бет" : "Главная"}</a><span>/</span><span>{t(page.metaTitle)}</span>
          </nav>
        ...
        <div className="service-hero-index" aria-hidden="true"><span>01</span><i /><span>AAA URBAN</span></div>
      </section>

      <section className="service-audience" aria-labelledby="audience-title">
        <div>
        ...
        <ul>
          {page.audience.map((item, index) => <li key={index}><Check size={17} />{t(item)}</li>)}
        </ul>
      </section>

      <section className="service-content-section" id="service-content">
      ...
      <section className="service-related-section">
        ...
        <div className="service-related-grid">
          {related.map((key, index) => {
            ...
            return (
              <a href={servicePath(key, lang)} key={key}>
                <span>{String(index + 1).padStart(2, "0")}</span>
